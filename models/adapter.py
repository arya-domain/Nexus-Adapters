...
class MultiAdapter(ModelMixin):
    ...
    Parameters:
        adapters (`List[NexusAdapter]`, *optional*, defaults to None):
            A list of `NexusAdapter` model instances.
    """

    def __init__(self, adapters: List["NexusAdapter"]):
        ...
        if len(adapters) == 1:
            raise ValueError("For a single adapter, please use the `NexusAdapter` class instead of `MultiAdapter`")
    ...
    def from_pretrained(cls, pretrained_model_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        while os.path.isdir(model_path_to_load):
            adapter = NexusAdapter.from_pretrained(model_path_to_load, **kwargs)
        ...
        if len(adapters) == 0:
            raise ValueError(
                f"No NexusAdapters found under {os.path.dirname(pretrained_model_path)}. Expected at least {pretrained_model_path + '_0'}."
            )

        return cls(adapters)


class NexusAdapter(ModelMixin, ConfigMixin):
    r"""
    ...
    def __init__(
        ...
        if adapter_type == "full_adapter":
            self.adapter = FullAdapter(in_channels, channels, num_res_blocks, downscale_factor, groups)
    ...
    def forward(self, x: torch.Tensor, txt: torch.Tensor) -> List[torch.Tensor]:
        r"""
...
class FullAdapter(nn.Module):
    r"""
    See [`NexusAdapter`] for more information.
    """
    ...
    def __init__(
        ...
        self.body = nn.ModuleList(
            [
                AdapterBlock(channels[0], channels[0], num_res_blocks, groups=groups, adapter=adapter),
                *[
                    AdapterBlock(channels[i - 1], channels[i], num_res_blocks, down=True, groups=groups, adapter=adapter)
...
class FullAdapterXL(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(channels)):
            if i == 1:
                self.body.append(AdapterBlock(channels[i - 1], channels[i], num_res_blocks, groups=groups, adapter=adapter))
            elif i == 2:
                self.body.append(AdapterBlock(channels[i - 1], channels[i], num_res_blocks, down=True, groups=groups, adapter=adapter))
            else:
                self.body.append(AdapterBlock(channels[i], channels[i], num_res_blocks, groups=groups, adapter=adapter))
...
class AdapterBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, num_res_blocks: int, down: bool = False, groups: int = 1, adapter: str = "full_adapter"):
        super().__init__()
        ...
        self.resnets = nn.Sequential(
            *[AdapterResnetBlock(out_channels, groups=groups) for _ in range(num_res_blocks)],
        )
        self.norm1 = nn.LayerNorm(out_channels, elementwise_affine=True, eps=1e-05)
        if adapter == "full_adapter":
            self.cross_attention = Attention(
                                        ...
                                        dropout=0.0,
                                        bias=False,
                                        cross_attention_dim=768,  # 2048 768
                                        upcast_attention=True,
                                        out_bias=True,
                                    )
        if adapter == "full_adapter_xl":
            self.cross_attention = Attention(
                                    query_dim=out_channels,
                                    heads=10,
                                    dim_head=64,
    ...
    def forward(self, x: torch.Tensor, txt: torch.Tensor) -> torch.Tensor:
        ...
        x = self.cross_attention(x, txt)

        return x

class HetConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, groups=2):
        super(HetConv, self).__init__()
        # Groupwise Convolution
        self.gwc = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, groups=groups, bias=False)
...
class AdapterResnetBlock(nn.Module):
    ...
    def __init__(self, channels: int, groups: int = 1):
        ...
        if groups == 1:
            ...
            self.block1 = HetConv(channels, channels, kernel_size=3, padding=1, groups=groups)
