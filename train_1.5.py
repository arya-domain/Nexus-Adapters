...
from models.adapter import NexusAdapter
...
def save_model_card(repo_id: str, image_logs: dict = None, base_model: str = None, repo_folder: str = None):
    ...
    model_description = f"""
# NexusAdapter-{repo_id}

These are NexusAdapter weights trained on {base_model} with new type of conditioning.
{img_str}
"""
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "A set of paths to the NexusAdapter conditioning image be evaluated every `--validation_steps`"
    ...
    parser.add_argument(
        ...
        type=str,
        default="sd_xl_train_NexusAdapter",
        help=(
    ...
    if args.resolution % 8 != 0:
        raise ValueError(
            "`--resolution` must be divisible by 8 for consistently sized encoded images between the VAE and the NexusAdapter encoder."
        )
...
def get_train_dataset(args, accelerator):
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_dataset(args.dataset_name, args.dataset_config_name, cache_dir=args.cache_dir, trust_remote_code=True)
    else:
...
def main(args):
    ...
    noise_scheduler = EulerDiscreteScheduler.from_pretrained(args.pretrained_model_name_or_path, 
                                                             subfolder="scheduler")
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = NexusAdapter.from_pretrained(os.path.join(input_dir, "NexusAdapter"))
    ...
    if unwrap_model(NexusAdapter).dtype != torch.float32:
        raise ValueError(
            f"Controlnet loaded as datatype {unwrap_model(NexusAdapter).dtype}. {low_precision_error_string}"
        )
    ...
    # Optimizer creation
    params_to_optimize = NexusAdapter.parameters()
    ...
    with accelerator.main_process_first():
        ...
        train_dataset = train_dataset.map(compute_embeddings_fn, batched=True, new_fingerprint=new_fingerprint, load_from_cache_file=False) # load_from_cache_file=False
    ...
    NexusAdapter, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        NexusAdapter, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(NexusAdapter):
                if args.pretrained_vae_model_name_or_path is not None:
                ...
                NexusAdapter_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype)
                down_block_additional_residuals = NexusAdapter(NexusAdapter_image, batch["prompt_ids"]) # , batch["prompt_ids"]
                ...
                if accelerator.sync_gradients:
                    params_to_clip = NexusAdapter.parameters()
            ...
            # logger.info(f"Epoch: {epoch} Total Loss in Epoch: {loss.item()}")
    ...
    if accelerator.is_main_process:
        NexusAdapter = unwrap_model(NexusAdapter)
        NexusAdapter.save_pretrained(args.output_dir)
